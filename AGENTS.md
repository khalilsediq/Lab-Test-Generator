...
- **Advanced Layout Responsiveness:** Improved Test Entry layout with adaptive grid breakpoints (`lg:`/`xl:`), fluid column widths, and smart-stacking strategies for tablets and small desktops. Specifically increased the `Input+Unit` column to `200px` for large screens to ensure long unit text (e.g., "mm induration") strictly aligns without overlapping into the reference range column. Provides superior usability even when side panels (Billing/Sidebar) are open.
- **Premium Sidebar UX:**
...
- **Advanced Test Parameter Management (NEW):**
  - **Selective Disabling:** Added a "Disable" toggle (eye icon) to every test parameter. Disabled rows remain in the UI (with lower opacity) for data entry but are automatically excluded from the final PDF and Print reports, allowing for tailored reports without deleting template data.
  - **Robust Custom Parameter Injection:** Entirely revamped the "Add Custom Parameter" inline editor. Technicians can now configure the full parameter spectrum directly within the test grid (Param Type, Qualitative Dropdown Options, General Notes, Explicit Male/Female Reference Ranges, and Custom Gender Ranges like 'Infant' or 'Child'). This eliminates the need for post-creation editing and guarantees precise report outputs.
  - **Parameter Trash Bin:** Added a full-featured Trash functionality for test parameters. Any deleted parameter (standard or custom) is moved to a dedicated **Test Parameters** tab in the Trash View, where it can be restored or permanently purged. Test parameter rows now dynamically and immediately clear from the UI upon deletion without requiring a reload.
  - **High-Reliability Persistence:** All parameter modifications (disabled states, custom additions, and trash status) are persisted via `localStorage` and synchronized across the `TestFields`, `ReportPreview`, `ReportTemplate`, and `TrashView` components.
- **React Stability & Performance:** Optimized `TestFields.jsx` by resolving React Hook rule violations, restructuring key-based logic, and completely replacing cascading synchronous `useEffect` calls (like list re-rendering on drag-and-drop) with high-efficiency `useMemo` derivations to cleanly decouple the rendering lifecycle.
